<template lang="pug">
  v-app.notion-theme(v-scroll='upBtnScroll', :dark='$vuetify.theme.dark', :class='$vuetify.rtl ? `is-rtl` : `is-ltr`')
    ...
    v-navigation-drawer(
      ...
      vue-scroll(:ops='scrollStyle')
        nav-sidebar(color='notion-sidebar', :dark='$vuetify.theme.dark', :items='sidebarDecoded', :nav-mode='navMode')
    ...
    v-main(ref='content')
      template(v-if='path !== `home`')
        v-toolbar.notion-breadcrumb-toolbar(flat, dense, v-if='$vuetify.breakpoint.smAndUp')
        ...
        v-divider
      v-container.notion-page-header.pa-0(fluid)
        v-row.page-header-section(no-gutters, align-content='center')
          v-col.page-col-content.is-page-header(
            ...
            .page-header-headings
              h1.notion-page-title {{title}}
              .notion-page-description(v-if='description') {{description}}
            .page-edit-shortcuts(
      ...
      v-divider
      v-container.notion-body(fluid, grid-list-xl)
        v-layout(row)
          v-flex.page-col-sd(
            ...
            v-card.page-toc-card.mb-5(flat, v-if='tocDecoded.length')
              .overline.pa-4.pb-0 {{$t('common:page.toc')}}
            ...
            v-card.page-tags-card.mb-5(flat, v-if='tags.length > 0')
              .pa-5
            ...
            v-card.page-comments-card.mb-5(flat, v-if='commentsEnabled && commentsPerms.read')
              .pa-5
            ...
            v-card.page-author-card.mb-5(flat)
              .pa-5
            ...
            v-card.page-shortcuts-card(flat)
              v-toolbar.notion-share-toolbar(flat, dense)
                v-spacer
                //- v-tooltip(bottom)
                //-   template(v-slot:activator='{ on }')
                ...
                v-tooltip(bottom)
                  template(v-slot:activator='{ on }')
                    v-btn(icon, tile, v-on='on', @click='print', :aria-label='$t(`common:page.printFormat`)')
                      v-icon(:color='printView ? `grey darken-3` : `grey`') mdi-printer
